...
namespace PaulaPires.Models
{
    public class ModeloHtml
    {
        ...
        public ModeloHtml(int id)
        {
            Clear();
            Load(id);
        }

        private ModeloHtml(DataRow pRow)
        {
            Clear();
            FillAttributes(pRow);
        }
        ...
        public void LoadbyPaginaId(int paginaId, int categoriaId, int subcategoriaId)
        {
            ...
            SQLServer sql = new SQLServer();
            DataSet result = sql.Select("spLoadConteudoHtmlByPaginaIdSite", CommandType.StoredProcedure, sqlParametros);

            if (DataHelper.HasTable(result) && DataHelper.HasRows(result))
                FillAttributes(DataHelper.GetFirstRow(result));
        }

        public void LoadbyPaginaHOME()
        {
            SQLServer sql = new SQLServer();
            DataSet result = sql.Select("spLoadConteudoHOMEsite");
        ...
        public bool Save()
        {
            ...
            sqlParametros.Add(new SqlParameter("@DescImagem", DescImagem));

            SQLServer sql = new SQLServer();
            Id = Convert.ToInt32(sql.ExecuteScalar("spSaveModeloHTML", CommandType.StoredProcedure, sqlParametros.ToArray()));
        ...
        public static List<ModeloHtml> List()
        {
            return GenericList("spListCategorias");
        }

        public static List<ModeloHtml> List(int secaoId)
        {
            SqlParameter[] sqlParametros =
            {
        ...
        public static List<ModeloHtml> ListMenuSite(int secaoId)
        {
            SqlParameter[] sqlParametros =
            {
                new SqlParameter("@secaoId", secaoId)
        ...
        public static List<ModeloHtml> GenericList(string pQuery)
        {
            var list = new List<ModeloHtml>();
            ...
            if (DataHelper.HasTable(result) && DataHelper.HasRows(result))
            {
                foreach (DataRow item in DataHelper.GetRows(result))
                    list.Add(new ModeloHtml(item));
            }

            return list;
        }

        public static List<ModeloHtml> GenericList(string pQuery, SqlParameter[] pSqlParametro)
        {
            var list = new List<ModeloHtml>();
