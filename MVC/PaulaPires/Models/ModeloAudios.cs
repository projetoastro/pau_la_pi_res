...
namespace PaulaPires.Models
{
    public class ModeloAudios
    {
        ...
        public ModeloAudios(int id)
        {
            Clear();
            Load(id);
        }

        private ModeloAudios(DataRow pRow)
        {
            Clear();
            FillAttributes(pRow);
        }
        ...
        public void LoadbyPagina(int paginaId)
        {
            ...
            SQLServer sql = new SQLServer();
            DataSet result = sql.Select("spLoadConteudoAudiosPaginaSite", CommandType.StoredProcedure, sqlParametros);
        ...
        public void LoadbyPaginaId(int paginaId, int categoriaId, int subcategoriaId)
        {
            ...
            SQLServer sql = new SQLServer();
            DataSet result = sql.Select("spLoadConteudoAudiosByPaginaIdSite", CommandType.StoredProcedure, sqlParametros);
        ...
        public bool Save()
        {
            ...
            SQLServer sql = new SQLServer();
            Id = Convert.ToInt32(sql.ExecuteScalar("spSaveModeloAudio", CommandType.StoredProcedure, sqlParametros.ToArray()));
        ...
        public static List<ModeloAudios> List()
        {
            return GenericList("spListCategorias");
        }

        public static List<ModeloAudios> List(int secaoId)
        {
            SqlParameter[] sqlParametros =
            {
        ...
        public static List<ModeloAudios> ListMenuSite(int secaoId)
        {
            SqlParameter[] sqlParametros =
            {
                new SqlParameter("@secaoId", secaoId)
        ...
        public static List<ModeloAudios> GenericList(string pQuery)
        {
            var list = new List<ModeloAudios>();
            ...
            if (DataHelper.HasTable(result) && DataHelper.HasRows(result))
            {
                foreach (DataRow item in DataHelper.GetRows(result))
                    list.Add(new ModeloAudios(item));
            }

            return list;
        }

        public static List<ModeloAudios> GenericList(string pQuery, SqlParameter[] pSqlParametro)
        {
            var list = new List<ModeloAudios>();
